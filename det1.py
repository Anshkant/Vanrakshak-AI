...
from strongsort.strong_sort import StrongSORT  # make sure this import path is correct
...
# Camera process
# -----------------------
def camera_process_main(cam_idx: int, rtsp_url: str, shm_names: List[str], ctrl_q: mp.Queue, write_idx_val: mp.Value, shutdown_ev: mp.Event):
    """
...
# -----------------------
def tracker_pipeline(cam_idx: int,
                     frame: np.ndarray,
                     h: int,
                     w: int,
                     yolo_future,
...
# -----------------------
def detection_process_main(cam_shm_names_all: List[List[str]],
                           ctrl_queues: List[mp.Queue],
                           jpeg_queues: List[mp.Queue],
                           counts_proxy,
...
# -----------------------
def draw_and_encode(cam_idx: int, frame: np.ndarray, tracks, track_hist: Dict, counts_proxy, jpeg_q: mp.Queue, h: int, w: int):
    """
    ...
    try:
        for track in tracks:
            try:
                ...
                cx = int((x1 + x2) / 2)
                cy = int((y1 + y2) / 2)
                cls_name = getattr(track, "det_class", "obj")
...
# -------- NEW: endpoint used by your HTML UI <img src="/video_feed?src="> ----------
@app.get("/video_feed")
async def video_feed(src: int = Query(0, ge=0)):
    """
    Unified video endpoint for UI:
    /video_feed?src=0 -> cam0
    /video_feed?src=1 -> cam1
    /video_feed?src=2 -> cam2
    /video_feed?src=3 -> cam3
    """
...
# -------- NEW: /start and /stop for your frontend ----------
@app.post("/start")
