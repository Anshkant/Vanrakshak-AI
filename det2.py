...
- One GPU detection thread (YOLO via ThreadPoolExecutor)
- StrongSORT on CPU threads (TorchReID-free version in strongsort_noreid)
- ThreadPoolExecutor for drawing + JPEG encoding
- FastAPI streaming endpoints / simple HTML UI
"""
...
from strongsort_noreid.strong_sort import StrongSORT
 # NOTE: use your TorchReID-free version
...
# StrongSORT params for our TorchReID-free StrongSORT
STRONGSORT_MAX_AGE = 30
STRONGSORT_MAX_IOU_DIST = 0.7
STRONGSORT_NN_BUDGET = 100
STRONGSORT_WEIGHTS = "osnet_x0_25_msmt17.pt"  # adjust path inside strongsort_noreid if needed
...
def detection_process_main(cam_shm_names_all, ctrl_queues, jpeg_queues, counts_proxy, shutdown_ev):
    ...
    # init StrongSORT trackers per camera (TorchReID-free StrongSORT)
    trackers = []
    for idx in range(len(cam_shm_names_all)):
        try:
            tr = StrongSORT(
                model_weights=STRONGSORT_WEIGHTS,
                device=device,
                max_age=STRONGSORT_MAX_AGE,
                max_iou_dist=STRONGSORT_MAX_IOU_DIST,
                nn_budget=STRONGSORT_NN_BUDGET
            )
        except Exception as e:
            print(f"[DETECT] StrongSORT init failed for cam{idx}: {e}")
            tr = None
        trackers.append(tr)
    ...
    try:
        for track in tracks:
            try:
                ...
                cx = int((x1 + x2) / 2)
                cy = int((y1 + y2) / 2)
                color = (0, 255, 0) if tid is not None else (0, 180, 255)
...
def mjpeg_stream_generator(cam_idx):
